...
def load_dataset(train_file_path: str, test_file_path: str) -> pd.DataFrame:
    ...
    df = df.drop(columns=["Ticket", "Embarked", "Cabin"])
    df.reset_index(drop=True, inplace=True)
    return df
    pass

load_dataset('data/train.csv', 'data/test.csv')
...
def get_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        total.append(df[col].isna().sum())
        percent.append((df[col].isna().sum())/len(df.index)*100)
    missing['Total'] = total
    missing['Percent'] = percent
    missing.sort_values(by='Total', inplace=True, ascending=False)
    #print(missing)
...
get_missing_values(load_dataset(os.path.join(data_dir, 'train.csv'),
...
def substitute_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Implement your own solution
    df1 = df.fillna({'Age':df['Age'].mean(), 'Fare':15})
    return df1
    pass
...
def get_correlation(df: pd.DataFrame) -> float:
    ...
    # Implement your own solution
    return df.corr(method='pearson').loc['Age', 'Fare']
    pass
...
def get_survived_per_class(df: pd.DataFrame,
    ...
    # Implement your own solution
    indexes = df[group_by_column_name].unique()
    sur = df.groupby(group_by_column_name)
    surr = df[[group_by_column_name, 'Survived']].groupby([group_by_column_name], as_index = True).mean().round(2).sort_values('Survived', ascending = False)
    s = surr["Survived"].squeeze()
    return s
    pass

get_survived_per_class(load_dataset(os.path.join(data_dir, 'train.csv'),
                      os.path.join(data_dir, 'test.csv')), group_by_column_name="Sex")
...
def get_outliers(df: pd.DataFrame) -> (int, pd.DataFrame):
    ...
    filtered = df.query('((@Q1 - 1.5 * @IQR) >= Fare | Fare >= (@Q3 + 1.5 * @IQR))')
    return (filtered.shape[0], filtered)
    pass

get_outliers(load_dataset(os.path.join(data_dir, 'train.csv'),
                      os.path.join(data_dir, 'test.csv')))
...
def create_new_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df1["Age_log"] = numpy.log(df1["Age"])
    df1["Sex"] = df1["Sex"].replace({"male": 0, "female": 1}).astype(int)
    return df1
    pass

create_new_features(load_dataset(os.path.join(data_dir, 'train.csv'),
                      os.path.join(data_dir, 'test.csv')))
...
def determine_survival(df: pd.DataFrame, n_interval: int, age: float,
    ...
    df["AgeInterval"] = pd.cut(df["Age"], n_interval)
    df1 = df[['AgeInterval', 'Sex', 'Survived']].groupby(['AgeInterval', 'Sex'], as_index=False).mean()
    print(df1.head())
    res = df1[(df1['AgeInterval'].isin([age])) & (df1['Sex'] == sex)]
    if (res['Survived'].empty): return numpy.NAN 
    else:
        return res['Survived'].iloc[0]
    pass


prob = determine_survival(load_dataset(os.path.join(data_dir, 'train.csv'),
                      os.path.join(data_dir, 'test.csv')), 20, age=33, sex="male")